    ...
    <>
        ...
        <div className="mx-auto max-w-[1700px] px-6 md:px-12">
          <Reveal>
            ...
            <h1 className="display mt-6 max-w-5xl text-6xl leading-[0.98] text-ivory md:text-[7rem]">
              Three regions.
              <span className="block italic text-stone">Dozens of hands. One standard.</span>
            </h1>
          ...
          <Reveal delay={0.15}>
            <div className="mt-16 grid grid-cols-2 gap-px overflow-hidden border border-charcoal-line bg-charcoal-line md:grid-cols-4">
              {[
                ...
                <div key={i} className="bg-charcoal-deep p-8 md:p-10">
                  <p className="display text-5xl text-gold md:text-6xl">{s.stat}</p>
      ...
      {CRAFTS.map((craft, ci) => {
        ...
        return (
          <section
            key={craft.key}
            className="border-t border-charcoal-line bg-charcoal py-24 md:py-36"
          >
            <div className="mx-auto grid max-w-[1700px] items-center gap-14 px-6 md:grid-cols-12 md:px-12">
                ...
                <div className="grid grid-cols-2 gap-6">
                  {imgs.map((im, i) => (
                    <Reveal key={im.src} delay={0.08 * i} className={i === 1 ? "mt-12" : ""}>
                      ...
                      />
                      <p className="eyebrow mt-3 text-stone">{im.caption}</p>
                    </Reveal>
                  ))}
                </div>
              </div>
              <div className={`md:col-span-5 ${flip ? "md:order-1 md:col-start-1" : "md:col-start-8"}`}>
                <Reveal>
                  ...
                  <h2 className="display mt-4 text-5xl text-ivory md:text-6xl">{craft.name}</h2>
                  <p className="lede mt-6 text-xl text-stone">{craft.line}</p>
                  <p className="mt-8 text-base leading-[1.95] text-ivory/80">{craft.detail}</p>
                </Reveal>
              </div>
            </div>
          </section>
      ...
      {/* Emotional close */}
      <section className="border-t border-charcoal-line bg-charcoal-deep py-28 text-center md:py-40">
        <Reveal>
          <p className="display mx-auto max-w-4xl px-6 text-4xl leading-[1.2] text-ivory md:text-6xl">
            When you acquire a SROJA piece, you are not paying for cloth.
            <span className="italic text-gold"> You are underwriting a craft&apos;s next generation.</span>
          </p>
          <Link
            href="/collections"
            className="eyebrow group mt-14 inline-flex items-center gap-4 border border-brass/60 px-10 py-5 text-gold transition-all duration-700 hover:border-gold hover:bg-gold hover:text-charcoal-deep"
          >
            View The Works
